...
- **依赖**：在 `deno.json` 与 `package.json` 中将 `@dreamer/esbuild` 提升至
  `^1.1.9`，使嵌套浏览器测试打包与 PostCSS / cssnano 的 peer 要求一致。
...
- **依赖**：`@dreamer/esbuild` 提升至 `^1.1.7`（JSR 与 npm 一致）。与 esbuild
  的修复对齐：在 **Bun** 下 `platform: "node"` 时走 esbuild 管线而非子进程
  `bun build`，减轻 Linux CI 上临时入口路径导致的偶发失败。
...
- **浏览器套件缓存键**：`suiteBrowserCache` 改为使用完整 `getFullSuiteName`
  路径作为键，不再只用 `">"` 分隔后的首段。Bun
  顺序执行多文件测试时套件路径可能被拼成
  `A > B`；旧键会让后续套件错误复用其它套件的 Playwright
  实例，引发导航挂死、外层用例超时及子进程被当作 dangling 清理。同一 `describe`
  内各 `it` 的完整路径仍相同，套件内复用浏览器行为不变。
- **`afterAll` 合成用例超时**：Bun 将 `afterAll` 注册为普通
  `test()`，默认超时很短；若清理含 `cleanupAllBrowsers()`
  或子进程结束较慢，合成用例会误超时。Deno/Bun 侧为该合成测试显式传入 60s 超时。
