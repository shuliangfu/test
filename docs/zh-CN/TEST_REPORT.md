...
本报告记录 `@dreamer/test` 的测试覆盖与结果。兼容 **Deno / Bun / Node.js**。

**测试日期**：2026-07-22 · **版本**：1.1.10 · **宿主**：`Deno.test` / `bun:test` / `node:test`

## 🎯 测试目标

1. 核心功能  2. 断言  3. Mock  4. **三端兼容**  5. 工具函数  6. 边界  
7. 套件/钩子  8. 浏览器  9. 客户端打包
...
| 运行时 | 命令 | 结果 |
| ------ | ---- | ---- |
| **Deno** | `deno test -A tests/` | **404 通过**，0 失败，2 ignored（~55s） |
| **Bun** | `bun test tests/` | 主路径绿；浏览器 e2e 偶发超时（Chrome 环境，非 Node 后端） |
| **Node** | `npm run test:node` | **277 通过**，0 失败，1 skipped（~1.5s） |

| 指标 | 数值 |
| ---- | ---- |
| 测试文件 | 21 + browser |
| Node 覆盖 | 非 browser 主套件 `tests/*.test.ts` |
| 代码覆盖 | 核心 API 全；浏览器以 Deno/Bun 为主 |
...
| `mock-document-comprehensive.test.ts`     | 7      | ✅ 全部通过           | Document/Cookie Mock（createCookieDocument，累积型） |
...
### 14.5 Document/Cookie Mock（createCookieDocument）（7 个）

- ✅ 返回带 cookie getter/setter 的对象
- ✅ 多 cookie 累积、不覆盖
- ✅ 按 name 更新同一 cookie
- ✅ 空值删除 cookie
- ✅ name/value 编解码
...
| mock-document.ts | 100% | Document/Cookie Mock 均有测试 |
...
- ✅ 断言系统、Mock、HTTP Mock、Document/Cookie
  Mock（createCookieDocument）、测试工具、套件选项、钩子选项均有全面测试
...
1. ✅ **功能完整**：已声明功能均正确实现
2. ✅ **稳定**：399 个用例通过，2 个按设计跳过（test.skip / skipIf），无失败
...
**测试报告生成时间**：2026-02-20 **测试执行环境**：

- Deno：最新稳定版
- Playwright：v1.58.2
- @dreamer/esbuild：v1.0.3
- **测试框架**：@dreamer/test@1.0.11
- **本次执行**：`deno test -A tests/` → ok | 399 passed | 0 failed | 2 ignored
  (16s)
